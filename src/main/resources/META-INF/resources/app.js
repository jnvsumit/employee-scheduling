...
function getShiftColor(shift, availabilityMap) {
  const shiftDate = JSJoda.LocalDateTime.parse(shift.start)
    .toLocalDate()
    .toString();
...
function renderSchedule(schedule) {
  ...
  const scheduleEnd = JSJoda.LocalDate.parse(scheduleStart)
    .plusDays(7)
    .toString();
  ...
  byEmployeeTimeline.setCustomTime(
    schedule.scheduleState.lastHistoricDate,
    "published"
  );
  byEmployeeTimeline.setCustomTime(
  ...
  byLocationTimeline.setCustomTime(
    schedule.scheduleState.lastHistoricDate,
    "published"
  );
  byLocationTimeline.setCustomTime(
  ...
  schedule.employees.forEach((employee, index) => {
    ...
    byEmployeeGroupDataSet.add({
      id: employee.name,
      content: employeeGroupElement.html(),
    });
  });
  ...
  schedule.shifts.forEach((shift, index) => {
    ...
    // LOCATION TO STORETYPE
    if (groups.indexOf(shift.storeType) === -1) {
      groups.push(shift.storeType);
      byLocationGroupDataSet.add({
        id: shift.storeType,
        content: shift.storeType,
      });
    }
    ...
    if (shift.employee == null) {
      ...
      const byLocationShiftElement = $('<div class="card-body p-2"/>')
        .append($(`<h5 class="card-title mb-2"/>)`).append("Unassigned"))
        .append(
          $("<div/>").append(
            $(
              `<span class="badge me-1 mt-1" style="background-color:#d3d7cf">${shift.requiredSkill}</span>`
            )
          )
        );
      ...
      const skillColor =
        shift.employee.skills.indexOf(shift.requiredSkill) === -1
          ? "#ef2929"
          : "#8ae234";
      const byEmployeeShiftElement = $('<div class="card-body p-2"/>')
        .append(
          $(`<h5 class="card-title mb-2"/>)`)
            ...
            .append(shift.storeType)
        )
        .append(
          $("<div/>").append(
            $(
              `<span class="badge me-1 mt-1" style="background-color:${skillColor}">${shift.requiredSkill}</span>`
            )
          )
        );
      const byLocationShiftElement = $('<div class="card-body p-2"/>')
        .append($(`<h5 class="card-title mb-2"/>)`).append(shift.employee.name))
        .append(
          $("<div/>").append(
            $(
              `<span class="badge me-1 mt-1" style="background-color:${skillColor}">${shift.requiredSkill}</span>`
  ...
  if (unassignedShiftsCount === 0) {
    ...
    unassignedShifts.append(
      $(`<p/>`).text(`There are ${unassignedShiftsCount} unassigned shifts.`)
    );
  }
...
function solve() {
  ...
  if (!startDate || !endDate) {
    swal({
      title: "Error!",
      text: "Please select schedule range",
      type: "error",
